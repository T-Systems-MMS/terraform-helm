...
resource "helm_release" "release" {
  ...
  dynamic "set_sensitive" {
    ...
    content {
      name  = local.helm_release[each.key].set_sensitive[set.key].name
      value = local.helm_release[each.key].set_sensitive[set.key].value
    }
  }
  ...
  dynamic "postrender" {
    ...
    content {
      binary_path  = local.helm_release[each.key].postrender[set.key].binary_path
    }
  }
}
